...
function displayVersionsAndQualifiers
{
	...
	echo -e "$(date +%d%b%y.%T) displayVersionsAndQualifiers.sh:${LINENO} |  \t OTS_ENV=$OTS_ENV"
	...
	echo
	echo
	echo -e "$(date +%d%b%y.%T) displayVersionsAndQualifiers.sh:${LINENO}  \t Note: Here are your localProducts directories..."
	echo
	ls ${MRB_SOURCE}/../ | grep localProducts
	echo
	echo

	echo -e "$(date +%d%b%y.%T) displayVersionsAndQualifiers.sh:${LINENO}  \t Note: below are the available otsdaq releases..."
	echo -e "$(date +%d%b%y.%T) displayVersionsAndQualifiers.sh:${LINENO}  \t ----------------------------"
	...
	echo -e "$(date +%d%b%y.%T) displayVersionsAndQualifiers.sh:${LINENO}  \t ----------------------------"
	echo -e "$(date +%d%b%y.%T) displayVersionsAndQualifiers.sh:${LINENO}  \t Note: above are the available otsdaq releases..."
	echo
	...
	echo -e "$(date +%d%b%y.%T) displayVersionsAndQualifiers.sh:${LINENO}  \t The latest otsdaq release is $LATEST_RELEASE"

	echo
	echo -e "$(date +%d%b%y.%T) displayVersionsAndQualifiers.sh:${LINENO}  \t Note: below are the available qualifiers for $LATEST_RELEASE.."
	echo -e "$(date +%d%b%y.%T) displayVersionsAndQualifiers.sh:${LINENO}  \t ----------------------------"
	#-s for silent, sed to remove closing </a>
	...
	echo -e "$(date +%d%b%y.%T) displayVersionsAndQualifiers.sh:${LINENO}  \t ----------------------------"
	echo -e "$(date +%d%b%y.%T) displayVersionsAndQualifiers.sh:${LINENO}  \t Note: above are the available qualifiers for $LATEST_RELEASE.."
	echo
	...
	echo
	echo -e "$(date +%d%b%y.%T) displayVersionsAndQualifiers.sh:${LINENO}  \t To explore the available qualifiers go here in your browser:"
	echo
	echo -e "\t\t\t\t https://scisoft.fnal.gov/scisoft/bundles/otsdaq"
	echo
	echo -e "$(date +%d%b%y.%T) displayVersionsAndQualifiers.sh:${LINENO}  \t ... then click the version, and manifest folder to view qualifiers."
	echo
	echo -e "$(date +%d%b%y.%T) displayVersionsAndQualifiers.sh:${LINENO}  \t To switch qualifiers, do the following: \n\n\t\t\t\t mrb newDev -f -v $LATEST_RELEASE -q $LATEST_QUAL:prof"
	echo
	echo -e "$(date +%d%b%y.%T) displayVersionsAndQualifiers.sh:${LINENO}  \t ...and replace '$LATEST_RELEASE' with your target version. and '$LATEST_QUAL:prof' with your qualifiers"
	echo -e "$(date +%d%b%y.%T) displayVersionsAndQualifiers.sh:${LINENO}  \t ...a new localProducts directory will be created, which you should use when you setup ots."
	echo -e "$(date +%d%b%y.%T) displayVersionsAndQualifiers.sh:${LINENO}  \t Note: Here are your localProducts directories..."
	echo
	ls ${MRB_SOURCE}/../ | grep localProducts
	echo
