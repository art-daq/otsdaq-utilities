...
def main(argv):
    ...
    for option, arg in options:
        if option == "-i":
            ...
            if not any(
                extension in arg for extension in extensionsToCheck
            ):  # not in arg: #check if it points to a file
    ...
    inputClassName = inputClassName.split("_interface", 1)[
        0
    ]  # Remove the '_interface' nomenclature
    ...
    with open(inputDirectory + "/" + inputFile, "rt") as fin:
        with open(
            outputDirectory + "/" + outputClassName + "_interface.cpp", "wt"
        ) as fout:
            for line in fin:
    ...
    with open(
        inputDirectory + "/" + inputClassName + inputHeaderExtension, "rt"
    ) as fin:
        with open(
            outputDirectory + "/" + outputClassName + inputHeaderExtension, "wt"
        ) as fout:
            for line in fin:
                fout.write(line.replace(inputClassName, outputClassName))
        ...
        with open(inputDirectory + "/" + "CMakeLists.txt", "rt") as fin:
            for line in fin:
                if firstLine in line:
                    ...
                    cMakeListsCopyBuffer += line.replace(
