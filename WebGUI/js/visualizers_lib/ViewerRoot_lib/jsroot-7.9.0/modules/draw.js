import { select as d3_select } from './d3.js';
import { loadScript, loadModules, findFunction, internals, settings, getPromise, isNodeJs, isObject, isFunc, isStr,
         ...
         clTPad, clTStyle, clTCanvas, clTGaxis, clTGeoVolume, kInspect, nsREX, nsSVG, atob_func } from './core.js';
import { clTStreamerInfoList, kBaseClass } from './io.js';
import { clTBranchFunc } from './tree.js';
import { BasePainter, compressSVG, svgToImage, _loadJSDOM } from './base/BasePainter.js';
import { ObjectPainter, cleanup, drawRawText, getElementCanvPainter, getElementMainPainter } from './base/ObjectPainter.js';
import { TPadPainter, clTButton } from './gpad/TPadPainter.js';
import { makePDF } from './base/makepdf.js';


async function import_more() { return import('./draw/more.js'); }

async function import_canvas() { return import('./gpad/TCanvasPainter.js'); }

async function import_tree() { return import('./draw/TTree.js'); }

async function import_h() { return import('./gui/HierarchyPainter.js'); }
...
drawFuncs = { lst: [
   ...
   { name: clTPave, icon: 'img_pavetext', class: () => import('./hist/TPavePainter.js').then(h => h.TPavePainter) },
   ...
   { name: /^TH1/, icon: 'img_histo1d', class: () => import('./hist/TH1Painter.js').then(h => h.TH1Painter), opt: ';hist;P;P0;E;E1;E2;E3;E4;E1X0;L;LF2;C;B;B1;A;TEXT;LEGO;same', ctrl: 'l', expand_item: fFunctions, for_derived: true },
   { name: clTProfile, icon: 'img_profile', class: () => import('./hist/TH1Painter.js').then(h => h.TH1Painter), opt: ';E0;E1;E2;p;AH;hist;projx;projxb;projxc=e;projxw', expand_item: fFunctions },
   { name: clTH2Poly, icon: 'img_histo2d', class: () => import('./hist/TH2Painter.js').then(h => h.TH2Painter), opt: ';COL;COL0;COLZ;LCOL;LCOL0;LCOLZ;LEGO;TEXT;same', expand_item: 'fBins', theonly: true },
   { name: 'TProfile2Poly', sameas: clTH2Poly },
   { name: 'TH2PolyBin', icon: 'img_histo2d', draw_field: 'fPoly', draw_field_opt: 'L' },
   { name: /^TH2/, icon: 'img_histo2d', class: () => import('./hist/TH2Painter.js').then(h => h.TH2Painter), opt: ';COL;COLZ;COL0;COL1;COL0Z;COL1Z;COLA;COL_POL;COL_ARR;BOX;BOX1;PROJ;PROJX1;PROJX2;PROJX3;PROJY1;PROJY2;PROJY3;PROJXY1;PROJXY2;PROJXY3;SCAT;TEXT;TEXTE;TEXTE0;CANDLE;CANDLE1;CANDLE2;CANDLE3;CANDLE4;CANDLE5;CANDLE6;CANDLEY1;CANDLEY2;CANDLEY3;CANDLEY4;CANDLEY5;CANDLEY6;VIOLIN;VIOLIN1;VIOLIN2;VIOLINY1;VIOLINY2;CONT;CONT1;CONT2;CONT3;CONT4;ARR;CHORD;SURF;SURF1;SURF2;SURF4;SURF6;E;A;LEGO;LEGO0;LEGO1;LEGO2;LEGO3;LEGO4;same', ctrl: 'lego', expand_item: fFunctions, for_derived: true },
   { name: clTProfile2D, sameas: clTH2, opt2: ';projxyb;projxyc=e;projxyw' },
   { name: /^TH3/, icon: 'img_histo3d', class: () => import('./hist/TH3Painter.js').then(h => h.TH3Painter), opt: ';SCAT;BOX;BOX2;BOX3;GLBOX1;GLBOX2;GLCOL', expand_item: fFunctions, for_derived: true },
   { name: clTProfile3D, sameas: clTH3 },
   { name: clTHStack, icon: 'img_histo1d', class: () => import('./hist/THStackPainter.js').then(h => h.THStackPainter), expand_item: 'fHists', opt: 'NOSTACK;HIST;COL;LEGO;E;PFC;PLC;PADS' },
   { name: clTPolyMarker3D, icon: 'img_histo3d', draw: () => import('./draw/draw3d.js').then(h => h.drawPolyMarker3D), direct: true, frame: '3d' },
   { name: clTPolyLine3D, icon: 'img_graph', draw: () => import('./draw/draw3d.js').then(h => h.drawPolyLine3D), direct: true, frame: '3d' },
   { name: 'TGraphStruct' },
   { name: 'TGraphNode' },
   { name: 'TGraphEdge' },
   { name: clTGraphTime, icon: 'img_graph', class: () => import('./hist/TGraphTimePainter.js').then(h => h.TGraphTimePainter), opt: 'once;repeat;first', theonly: true },
   { name: clTGraph2D, icon: 'img_graph', class: () => import('./hist/TGraph2DPainter.js').then(h => h.TGraph2DPainter), opt: ';P;PCOL', theonly: true },
   ...
   { name: clTGraphPolargram, icon: 'img_graph', class: () => import('./draw/TGraphPolarPainter.js').then(h => h.TGraphPolargramPainter), theonly: true },
   { name: clTGraphPolar, icon: 'img_graph', class: () => import('./draw/TGraphPolarPainter.js').then(h => h.TGraphPolarPainter), opt: ';F;L;P;PE', theonly: true },
   { name: /^TGraph/, icon: 'img_graph', class: () => import('./hist2d/TGraphPainter.js').then(h => h.TGraphPainter), opt: ';L;P' },
   { name: 'TEfficiency', icon: 'img_graph', class: () => import('./hist/TEfficiencyPainter.js').then(h => h.TEfficiencyPainter), opt: ';AP;A4P;B' },
   { name: clTCutG, sameas: clTGraph },
   { name: /^RooHist/, sameas: clTGraph },
   { name: /^RooCurve/, sameas: clTGraph },
   { name: /^RooEllipse/, sameas: clTGraph },
   { name: 'TScatter', icon: 'img_graph', class: () => import('./hist2d/TScatterPainter.js').then(h => h.TScatterPainter), opt: ';A' },
   { name: 'RooPlot', icon: 'img_canvas', draw: () => import('./hist/TGraphTimePainter.js').then(h => h.drawRooPlot) },
   { name: 'TRatioPlot', icon: 'img_mgraph', class: () => import('./draw/TRatioPlotPainter.js').then(h => h.TRatioPlotPainter), opt: '' },
   { name: clTMultiGraph, icon: 'img_mgraph', class: () => import('./hist/TMultiGraphPainter.js').then(h => h.TMultiGraphPainter), opt: ';ac;l;p;3d;pads', expand_item: 'fGraphs' },
   { name: clTStreamerInfoList, icon: 'img_question', draw: () => import_h().then(h => h.drawStreamerInfo) },
   { name: 'TWebPainting', icon: 'img_graph', class: () => import('./draw/TWebPaintingPainter.js').then(h => h.TWebPaintingPainter) },
   ...
   { name: clTF1, icon: 'img_tf1', class: () => import('./hist/TF1Painter.js').then(h => h.TF1Painter), opt: ';L;C;FC;FL' },
   { name: clTF12, sameas: clTF1 },
   { name: clTF2, icon: 'img_tf2', class: () => import('./hist/TF2Painter.js').then(h => h.TF2Painter), opt: ';BOX;ARR;SURF;SURF1;SURF2;SURF4;SURF6;LEGO;LEGO0;LEGO1;LEGO2;LEGO3;LEGO4;same' },
   { name: clTF3, icon: 'img_histo3d', class: () => import('./hist/TF3Painter.js').then(h => h.TF3Painter), opt: ';SURF' },
   { name: clTSpline3, icon: 'img_tf1', class: () => import('./draw/TSplinePainter.js').then(h => h.TSplinePainter) },
   ...
   { name: clTLine, icon: 'img_graph', class: () => import('./draw/TLinePainter.js').then(h => h.TLinePainter) },
   { name: 'TArrow', icon: 'img_graph', class: () => import('./draw/TArrowPainter.js').then(h => h.TArrowPainter) },
   { name: clTPolyLine, icon: 'img_graph', class: () => import('./draw/TPolyLinePainter.js').then(h => h.TPolyLinePainter), opt: ';F' },
   { name: 'TCurlyLine', sameas: clTPolyLine },
   { name: 'TCurlyArc', sameas: clTPolyLine },
   { name: 'TParallelCoord', icon: 'img_graph', dummy: true },
   { name: clTGaxis, icon: 'img_graph', class: () => import('./draw/TGaxisPainter.js').then(h => h.TGaxisPainter) },
   { name: clTBox, icon: 'img_graph', class: () => import('./draw/TBoxPainter.js').then(h => h.TBoxPainter), opt: ';L' },
   ...
   { name: 'TASImage', icon: 'img_mgraph', class: () => import('./draw/TASImagePainter.js').then(h => h.TASImagePainter), opt: ';z' },
   ...
   { name: clTTree, icon: 'img_tree', get_expand: () => import('./tree.js').then(h => h.treeHierarchy), draw: () => import_tree().then(h => h.drawTree), dflt: 'expand', opt: 'player;testio', shift: kInspect },
...
/** @summary Set default draw option for provided class
  * @example
  import { setDefaultDrawOpt } from 'https://root.cern/js/latest/modules/draw.js';
...
/** @summary Draw object in specified HTML element with given draw options.
  ...
  * @example
  * import { openFile } from 'https://root.cern/js/latest/modules/io.js';
  * import { draw } from 'https://root.cern/js/latest/modules/draw.js';
...
async function draw(dom, obj, opt) {
   ...
   function performDraw() {
      let promise, painter;
      if (handle.direct === 'v7') {
         promise = import('./gpad/RCanvasPainter.js').then(v7h => {
   ...
   if (isFunc(handle.class)) {
      ...
      if (isStr(handle.script)) {
         ...
         else if (handle.script.indexOf('.js') > 0)
...
/** @summary Redraw object in specified HTML element with given draw options.
  ...
  * @example
  * import { openFile } from 'https://root.cern/js/latest/modules/io.js';
  * import { draw, redraw } from 'https://root.cern/js/latest/modules/draw.js';
  * let file = await openFile('https://root.cern/js/files/hsimple.root');
  * let obj = await file.readObject('hpxpy;1');
...
import_geo = async function() {
   return import('./geom/TGeoPainter.js').then(geo => {
...
import_v7 = async function(arg) {
   return import('./gpad/RCanvasPainter.js').then(h => {
      // only now one can draw primitives in the canvas
      assignPadPainterDraw(h.RPadPainter);
      switch (arg) {
         case 'more': return import('./draw/v7more.js');
         case 'pave': return import('./hist/RPavePainter.js');
         case 'rh1': return import('./hist/RH1Painter.js');
         case 'rh2': return import('./hist/RH2Painter.js');
         case 'rh3': return import('./hist/RH3Painter.js');
      }
      return h;
   });
};
