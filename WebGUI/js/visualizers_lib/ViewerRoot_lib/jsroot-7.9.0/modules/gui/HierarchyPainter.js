import { version, gStyle, httpRequest, create, createHttpRequest, loadScript, loadModules, decodeUrl,
         ...
         prROOT, clTList, clTMap, clTObjString, clTKey, clTFile, clTText, clTLatex, clTColor, clTStyle, kInspect, isPromise } from '../core.js';
import { select as d3_select } from '../d3.js';
import { openFile, kBaseClass, clTStreamerInfoList, clTDirectory, clTDirectoryFile, nameStreamerInfo, addUserStreamer } from '../io.js';
import { getRGBfromTColor } from '../base/colors.js';
import { prJSON, BasePainter, getElementRect, _loadJSDOM, getTDatime, convertDate } from '../base/BasePainter.js';
import { getElementMainPainter, getElementCanvPainter, cleanup, ObjectPainter } from '../base/ObjectPainter.js';
import { createMenu } from './menu.js';
import { getDrawSettings, getDrawHandle, canDrawHandle, addDrawFunc, draw, redraw } from '../draw.js';
import { BatchDisplay, GridDisplay, TabsDisplay, FlexibleDisplay, BrowserLayout, getHPainter, setHPainter } from './display.js';
import { showProgress, ToolbarIcons, registerForResize, injectStyle, saveFile } from './utils.js';
...
/**
  ...
  *
  * @example
  * import { HierarchyPainter } from 'https://root.cern/js/latest/modules/gui/HierarchyPainter.js';
...
class HierarchyPainter extends BasePainter {
  ...
  /** @summary Drop item on specified element for drawing
   ...
   async importModule(module) {
      switch (module) {
         case 'core': return import('../core.js');
         case 'draw_tree': return import('../draw/TTree.js');
      ...
      const adoptHierarchy = async result => {
         ...
         this.forEachItem(item => {
            ...
            if (item._autoload) {
               ...
               arr.forEach(name => {
                  if ((name.length > 4) && (name.lastIndexOf('.js') === name.length - 4))
   ...
   async loadScripts(scripts, modules, use_inject) {
      ...
      if (use_inject && scripts.indexOf('.js') > 0)
