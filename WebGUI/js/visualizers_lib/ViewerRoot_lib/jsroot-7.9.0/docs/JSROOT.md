...
Major JSROOT functions are located in `main.js` module and can be imported like:

```javascript
<script type='module'>
   import { openFile, draw } from 'https://root.cern/js/latest/modules/main.js';
...
```javascript
<script type='module'>
   import { httpRequest, draw } from 'http://your_root_server:8080/jsrootsys/modules/main.js';
...
```javascript
<script type='module'>
   import { HierarchyPainter } from 'https://root.cern/js/latest/modules/gui.js';
...
```javascript
import { gStyle } from 'https://root.cern/js/latest/modules/main.js';
...
```javascript
import { settings } from 'https://root.cern/js/latest/modules/main.js';
...
```javascript
import { httpRequest } from 'https://root.cern/js/latest/modules/main.js';
...
```javascript
import { parse } from 'https://root.cern/js/latest/modules/main.js';
...
```javascript
import { draw } from 'https://root.cern/js/latest/modules/main.js';
...
```javascript
import { httpRequest, draw, redraw, resize, cleanup } from 'https://root.cern/js/latest/modules/main.js';
...
import { openFile } from 'https://root.cern/js/latest/modules/io.js';
import { draw } from 'https://root.cern/js/latest/modules/draw.js';
import { TSelector, treeProcess } from 'https://root.cern/js/latest/modules/tree.js';
...
```javascript
import { build } from './path_to_jsroot/modules/geom/TGeoPainter.js';
...
```javascript
import { produceRenderOrder } from './path_to_jsroot/modules/geom/TGeoPainter.js';
...
```javascript
<script type="module">
   import { loadOpenui5 } from 'path_to_jsroot/modules/main.js';
...
as `/jsrootsys/modules/main.js`. And then trying to access files from local disk, one should specify `/currentdir/` folder:
...
### Migration v6 -> v7
   ...
   * Core functionality should be imported from `main.js` module like:

```javascript
import { create, parse, createHistogram, redraw } from 'https://root.cern/js/7.0.0/modules/main.js';
```
...
```javascript
import { getHPainter } from 'https://root.cern/js/7.0.0/modules/main.js';
let hpainter = getHPainter();
```

   * All math functions previously available via `JSROOT.Math` should be imported from `base/math.js` module:

```javascript
import * as math from 'https://root.cern/js/7.0.0/modules/base/math.js';
```

   * Indication of batch mode `JSROOT.batch_mode` should be accessed via functions:

```javascript
import { isBatchMode, setBatchMode } from 'https://root.cern/js/7.0.0/modules/main.js';
