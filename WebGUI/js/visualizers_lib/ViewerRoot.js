...
ViewerRoot.refreshTransparency = function(i) {
    if (!ViewerRoot.rootIsTransparentArr[i]) return;

    var target = ViewerRoot.rootElArr[i] && ViewerRoot.rootElArr[i].children[0];
    if (!target) return;
    ...
    // only first rect, which is usually the background
