                               ...
                               const WebUsers::RequestUserInfo& userInfo)
{
	    ...
	    "getRoot")  // ################################################################################################################
	{
		...
		if(!isLiveDQM)  // If it is not from LIVE_DQM
		{
			...
			// it is a directory
			if((tObject = rootFile->Get(rootDirectoryName.c_str())) != nullptr || //for backwards compatibility
				(rootDirectoryName.size() && rootDirectoryName[0] == '/' &&	//for newer root, remove leading '/'
					(tObject = rootFile->Get(rootDirectoryName.substr(1).c_str())) != nullptr))
			{
			        ...
			        nullptr)
			{
				...
				while((key = (TKey*)next()))
				{
					...
					if(TTEST(50)) //enable to view json content of each object in directory
					{
						tObject = key->ReadObj();
						...
						__SUP_COUT__ << "Returning directory object from file '" << tObject->GetName()
									<< "' of class '" << tObject->ClassName() << __E__;
